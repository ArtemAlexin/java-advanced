...
public class JarImplementor implements JarImpler {
     ...
     */
    private static final ConstructorGenerator CONSTRUCTOR_GENERATOR = new ConstructorGenerator(JarImplementor::generateClassName);
    /**
    ...
    private static Path getPath(Path path, Class<?> token) {
        return getPath(path, token, JarImplementor::getClassNameImpl);
    }
     ...
     * Uses {@link JarImplementor#METHOD_GENERATOR} to create implementation of methods.
     ...
     * Invokes {@link AbstractExecutableGenerator#createExecutable} to create {@code String representation} of method.
     * Uses {@link JarImplementor#CONSTRUCTOR_GENERATOR} to create constructor implementation.
     *
     * @param token  type token which represents class or interface that must be implemented.
     * @param writer {@code BufferedWriter} where.
     * @throws IOException if {@link BufferedWriter#write(String)} throws an {@code IOException}.
     ...
     * Uses {@link JarImplementor#FILE_VISITOR} and invokes {@link Files#walkFileTree} to clean file tree.
     *
    ...
    private void build(Class<?> token, Path tmpDirectory, Path jarFile) throws ImplerException {
        ...
        try (JarOutputStream outputStream = new JarOutputStream(Files.newOutputStream(jarFile), manifest)) {
            outputStream.putNextEntry(new ZipEntry(
                    getNameForJar(token)));
            Files.copy(getPath(tmpDirectory, token, JarImplementor::getCompiledClassName), outputStream);
        } catch (IOException e) {
    ...
    private void compile(Class<?> token, Path tmpDirectory) throws ImplerException {
        ...
        String[] args = {
                ...
                tmpDirectory.resolve(getFilePath(token, JarImplementor::getClassNameImpl)).toString()};
    ...
    public static void main(String[] args) {
        try {
            ...
            JarImplementor generator = new JarImplementor();
            if (args.length == 2) {
